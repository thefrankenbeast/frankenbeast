import type { PulseResult, Flag } from '../core/types.js';
...
function checkWatchlist(watchlist: readonly WatchlistItem[]): Flag[] {
...
function checkDeepReviewHour(clock: () => Date, deepReviewHour: number): Flag[] {
  const hour = clock().getUTCHours();
  if (hour !== deepReviewHour) return [];
  return [{
    source: 'deep_review',
...
async function checkGitStatus(executor: () => Promise<GitStatusResult>): Promise<Flag[]> {
  try {
...
async function checkTokenSpend(
  ...
  try {
    ...
    if (spend.totalCostUsd <= threshold) return [];
    return [{
      source: 'token_spend',
...
export class DeterministicChecker {
  private readonly deps: DeterministicCheckerDeps;

  constructor(deps: DeterministicCheckerDeps) {
    this.deps = deps;
  }

  async check(watchlist: readonly WatchlistItem[]): Promise<PulseResult> {
