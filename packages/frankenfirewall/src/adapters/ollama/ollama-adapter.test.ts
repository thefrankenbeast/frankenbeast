...
import type { ConformanceFixtures } from "../conformance/adapter-conformance.js";
import { SIMPLE_TEXT_REQUEST, TOOL_CALL_REQUEST } from "../conformance/conformance-fixtures.js";
...
describe("Adapter Conformance: OllamaAdapter", () => {
  ...
  it("passes conformance for text responses", () => {
    ...
    const nonToolFailures = result.failures.filter(
      (f) => !f.includes("tool response: tool_calls array is empty"),
    );
...
describe("OllamaAdapter", () => {
  ...
  it("transformRequest sets stream to false", () => {
    const adapter = new OllamaAdapter({ model: "llama3.1" });
    const request = adapter.transformRequest(SIMPLE_TEXT_REQUEST) as { stream: boolean };
    expect(request.stream).toBe(false);
  });

  it("transformRequest maps max_tokens to options.num_predict", () => {
    const adapter = new OllamaAdapter({ model: "llama3.1" });
    const request = adapter.transformRequest(SIMPLE_TEXT_REQUEST) as {
