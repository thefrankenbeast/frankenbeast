...
function makeBlockedResponse(requestId, violations) {
    return {
        schema_version: 1,
        id: requestId,
...
export async function runPipeline(request, adapter, config, options = {}) {
    const allViolations = [];
    // -------------------------------------------------------------------------
    ...
    try {
        ...
        const violation = isGuardrailViolation(err)
            ? err
            : {
                code: "ADAPTER_ERROR",
...
function isGuardrailViolation(err) {
