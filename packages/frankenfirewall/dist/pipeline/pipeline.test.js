...
describe("runPipeline — inbound blocks", () => {
    ...
    it("ProjectAlignmentChecker block returns all violations", async () => {
        const adapter = makeAdapter();
        const result = await runPipeline({ ...BASE_REQUEST, provider: "local-ollama" }, adapter, BASE_CONFIG);
...
describe("runPipeline — PII masking", () => {
    it("PII in request is masked before reaching adapter", async () => {
        ...
        await runPipeline({ ...BASE_REQUEST, messages: [{ role: "user", content: "Email me at spy@secret.com" }] }, adapter, BASE_CONFIG);
