...
describe("ProjectAlignmentChecker", () => {
    ...
    it("BLOCK — provider not in allowed_providers", () => {
        const result = checkProjectAlignment({ ...BASE_REQUEST, provider: "local-ollama" }, BASE_CONFIG);
    ...
    it("BLOCK — estimated tokens exceed budget ceiling", () => {
        ...
        const result = checkProjectAlignment({ ...BASE_REQUEST, messages: [{ role: "user", content: longContent }] }, BASE_CONFIG);
        expect(result.passed).toBe(false);
        if (!result.passed) {
    ...
    it("BLOCK — tool not in Skill Registry", () => {
        const registry = { hasSkill: () => false };
        const result = checkProjectAlignment({ ...BASE_REQUEST, tools: [{ name: "unknown_tool", description: "x", input_schema: {} }] }, BASE_CONFIG, registry);
        expect(result.passed).toBe(false);
        if (!result.passed) {
    ...
    it("PASS — tool exists in Skill Registry", () => {
        const registry = { hasSkill: (name) => name === "get_weather" };
        const result = checkProjectAlignment({ ...BASE_REQUEST, tools: [{ name: "get_weather", description: "x", input_schema: {} }] }, BASE_CONFIG, registry);
        expect(result.passed).toBe(true);
    });
    it("collects multiple violations in a single pass", () => {
        const longContent = "a".repeat(200_000);
        const result = checkProjectAlignment({ ...BASE_REQUEST, provider: "local-ollama", messages: [{ role: "user", content: longContent }] }, BASE_CONFIG);
        expect(result.passed).toBe(false);
        if (!result.passed) {
...
//# sourceMappingURL=project-alignment-checker.test.js.map