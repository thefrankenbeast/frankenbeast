...
function makeAdapter(model = "claude-sonnet-4-6") {
...
describe("ClaudeAdapter.transformRequest", () => {
    it("maps system prompt to Anthropic top-level system field", () => {
        ...
        const result = adapter.transformRequest({ ...BASE_REQUEST, system: "You are helpful." });
        expect(result["system"]).toBe("You are helpful.");
    });
    it("omits system field when not provided", () => {
        const adapter = makeAdapter();
        const result = adapter.transformRequest(BASE_REQUEST);
...
//# sourceMappingURL=claude-adapter.test.js.map