...
export function writeProgress(line, opts) {
...
export class CliSkillExecutor {
    ...
    logger;
    defaultMartinConfig;
    constructor(martin, git, observer, verifyCommand, commitMessageFn, logger, defaultMartinConfig) {
        this.martin = martin;
        this.git = git;
    ...
    async recoverDirtyFiles(taskId, stage, checkpoint, logger) {
    ...
    async execute(skillId, input, config, checkpoint, taskId) {
        ...
        try {
            ...
            throw new Error(`Git isolation failed for chunk "${chunkId}": ${err instanceof Error ? err.message : String(err)}`);
        }
        ...
        const wrappedConfig = {
            ...
            onProviderAttempt: (provider, iteration) => {
                ...
                writeProgress(formatIterationProgress({ chunkId, iteration, maxIterations: wrappedConfig.maxIterations }), { final: false });
            ...
            onIteration: (iteration, result) => {
                ...
                writeProgress(formatIterationProgress({
                    chunkId,
                    iteration,
                ...
                // Record token usage
                this.observer.recordTokenUsage(iterSpan, {
        ...
        try {
            ...
            this.observer.endSpan(chunkSpan, { status: 'error', errorMessage: String(err) });
            throw new Error(`MartinLoop failed for chunk "${chunkId}": ${err instanceof Error ? err.message : String(err)}`);
        }
        ...
        if (!mergeResult.merged && mergeResult.conflicted && mergeResult.conflictFiles?.length) {
            mergeResult = await this.attemptConflictResolution(chunkId, mergeResult, commitMessage, wrappedConfig, chunkSpan);
        }
        const postTokens = this.observer.counter.grandTotal();
    ...
    async attemptConflictResolution(chunkId, mergeResult, commitMessage, parentConfig, chunkSpan) {
...
//# sourceMappingURL=cli-skill-executor.js.map