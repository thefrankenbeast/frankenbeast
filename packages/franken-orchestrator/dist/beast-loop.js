...
export class BeastLoop {
    ...
    async run(input) {
        ...
        try {
            ...
            await runPlanning(ctx, this.deps.planner, this.deps.critique, this.config, logger, this.deps.graphBuilder);
            logger.info('BeastLoop: phase end', { phase: 'planning' });
            // Phase 3: Execution
            logger.info('BeastLoop: phase start', { phase: 'execution' });
            const outcomes = await runExecution(ctx, this.deps.skills, this.deps.governor, this.deps.memory, this.deps.observer, this.deps.mcp, logger, this.deps.cliExecutor, this.deps.checkpoint, this.deps.refreshPlanTasks);
            logger.info('BeastLoop: phase end', { phase: 'execution' });
            // Phase 4: Closure
            logger.info('BeastLoop: phase start', { phase: 'closure' });
            const result = await runClosure(ctx, this.deps.observer, this.deps.heartbeat, this.config, outcomes, logger, this.deps.prCreator);
...
//# sourceMappingURL=beast-loop.js.map