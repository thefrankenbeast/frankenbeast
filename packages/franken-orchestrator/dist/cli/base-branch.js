 ...
 */
export function detectCurrentBranch(workingDir) {
    try {
 ...
 */
export async function resolveBaseBranch(workingDir, cliOverride, io) {
    if (cliOverride) {
        return cliOverride;
    }
    ...
    const answer = await io.ask(`You're on branch "${current}". Are you sure you want to target this as your base branch? (y/n)`);
