...
const defaultExec = (cmd) => execSync(cmd, { encoding: 'utf-8', stdio: 'pipe' });
export class PrCreator {
    config;
    exec;
    llm;
    constructor(config, exec = defaultExec, llm) {
        this.config = {
    ...
    async generateCommitMessage(diffStat, chunkObjective) {
        if (!this.llm)
            return null;
        try {
            const prompt = [
    ...
    async generatePrDescription(commitLog, diffStat, result, issueNumber) {
        if (!this.llm)
            return null;
        try {
            const promptLines = [
            ...
            promptLines.push('', 'Respond in this exact format:', 'TITLE: <title here>', 'BODY:', '<body here>');
    ...
    async create(result, logger, options) {
        ...
        try {
            const output = this.exec(`gh pr create --base ${targetBranch} --title ${shellEscape(title)} --body ${shellEscape(body)}`);
    ...
    async tryGeneratePrFromLlm(result, logger, issueNumber) {
        if (!this.llm)
            return null;
        try {
            const commitLog = this.safeExec(`git log ${this.config.targetBranch}..HEAD --oneline`, logger) ?? '';
            const diffStat = this.safeExec(`git diff --stat ${this.config.targetBranch}..HEAD`, logger) ?? '';
    ...
    gatherGitContext(targetBranch, logger) {
        const diffStat = this.safeExec(`git diff --stat ${targetBranch}...HEAD`, logger) ?? '';
        const logOutput = this.safeExec(`git log --oneline ${targetBranch}...HEAD`, logger) ?? '';
...
function buildTitle(projectId, outcomes) {
    // Extract unique chunk names (strip impl:/harden: prefixes, deduplicate)
    const chunkNames = [...new Set(outcomes.map(o => o.taskId.replace(/^(impl|harden):/, '')))];
    const prefix = 'feat: ';
    const maxLength = 70;
...
function buildBody(result, outcomes, gitContext) {
...
function buildChunkDescriptions(outcomes, gitContext) {
    ...
    for (const o of outcomes) {
        ...
        if (isImpl)
            entry.impl = o.status === 'success';
        if (isHarden)
            entry.harden = o.status === 'success';
...
function getIterationCount(outcome) {
...
function formatIterations(outcome) {
...
function parsePrDescription(raw) {
...
function buildFallbackCommitMessage(chunkObjective) {
...
function appendIssueRef(body, issueNumber) {
