...
function findTriage(triages, issueNumber) {
    return triages.find(t => t.issueNumber === issueNumber);
}
export class IssueRunner {
    async run(config) {
        const { issues, triageResults, graphBuilder, executor, git, prCreator, checkpoint, logger, budget, baseBranch, noPr, repo, } = config;
        if (issues.length === 0)
            return [];
    ...
    async processIssue(issue, triage, config, cumulativeTokens, budgetTokens) {
        ...
        if (!noPr && prCreator) {
            try {
                ...
                logger?.warn(`[issues] PR creation failed for issue #${issue.number}: ${err instanceof Error ? err.message : String(err)}`, undefined, 'issues');
...
//# sourceMappingURL=issue-runner.js.map