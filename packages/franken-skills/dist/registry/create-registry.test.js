...
function makeCliSuccess(stdout) {
...
describe("createRegistry + sync()", () => {
    ...
    it("sync() with malformed CLI stdout — throws SkillRegistryError, registry stays empty", async () => {
        ...
        const registry = createRegistry({ localSkillsDir: "/tmp/nonexistent-skills" });
        await expect(registry.sync()).rejects.toSatisfy((e) => e instanceof SkillRegistryError && e.code === "PARSE_ERROR");
        expect(registry.isSynced()).toBe(false);
    });
    it("sync() with CLI failure — throws SkillRegistryError(CLI_FAILURE)", async () => {
        makeCliFailure();
        const registry = createRegistry({ localSkillsDir: "/tmp/nonexistent-skills" });
        await expect(registry.sync()).rejects.toSatisfy((e) => e instanceof SkillRegistryError && e.code === "CLI_FAILURE");
    });
    ...
    it("debug log emitted per skill after sync()", async () => {
        ...
        expect(debugSpy).toHaveBeenCalledWith("[SkillRegistry] Registered skill", expect.objectContaining({ skill_id: "deploy-to-vercel", source: "GLOBAL" }));
    ...
    it("getSkill() for unknown id triggers scaffold warn — still returns undefined", async () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("does-not-exist"), expect.anything());
        warnSpy.mockRestore();
    });
...
//# sourceMappingURL=create-registry.test.js.map