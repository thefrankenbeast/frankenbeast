...
export class SkillRegistry {
    ...
    assertSynced() {
        if (!this.synced) {
            throw new SkillRegistryError("REGISTRY_NOT_SYNCED", "Registry has not been synced. Await sync() before querying.");
        }
    }
    /**
    ...
    register(contract) {
        ...
        if (!validation.ok) {
            throw new SkillRegistryError("INVALID_CONTRACT", `Cannot register skill "${contract.skill_id}": ${validation.errors.map((e) => e.message).join("; ")}`, contract.skill_id);
        }
        const existing = this.store.get(contract.skill_id);
        if (existing) {
            if (contract.metadata.source === "LOCAL") {
                console.info(`[SkillRegistry] Local skill "${contract.skill_id}" overrides global entry.`, { skill_id: contract.skill_id, source: "LOCAL" });
     ...
     */
    static resolveSkills(globals, locals) {
        const skills = new Map();
        const overrides = new Set();
        for (const skill of globals) {
            if (skills.has(skill.skill_id)) {
                console.error(`[SkillRegistry] Duplicate skill_id "${skill.skill_id}" in global source: keeping first.`, { skill_id: skill.skill_id, source: "GLOBAL" });
                continue;
            }
        ...
        for (const skill of locals) {
            if (skills.has(skill.skill_id) && skills.get(skill.skill_id)?.metadata.source === "LOCAL") {
                console.error(`[SkillRegistry] Duplicate skill_id "${skill.skill_id}" in local source: keeping first.`, { skill_id: skill.skill_id, source: "LOCAL" });
                continue;
            }
            if (skills.has(skill.skill_id)) {
