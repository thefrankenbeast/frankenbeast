...
describe("DiscoveryService", () => {
    ...
    it("logs and skips duplicate skill_ids — first entry kept", async () => {
        ...
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining("run-tests"), expect.anything());
        logSpy.mockRestore();
    });
    it("propagates SkillRegistryError from CLI (does not swallow)", async () => {
        ...
        const service = new DiscoveryService(cli);
        await expect(service.discover()).rejects.toSatisfy((e) => e instanceof SkillRegistryError && e.code === "CLI_FAILURE");
    });
});
