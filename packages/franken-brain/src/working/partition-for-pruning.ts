 ...
 */
export function partitionForPruning(turns: WorkingTurn[]): PartitionResult {
  const preservedIds = new Set<string>();

  // Rule 1: pinned
  for (const t of turns) {
    if (t.pinned === true) preservedIds.add(t.id);
  }
  ...
  const preserved: WorkingTurn[] = [];
  const candidates: WorkingTurn[] = [];
