...
const collectionName = (projectId) => `project-${projectId}`;
export class SemanticMemoryStore {
    client;
    embedder;
    // Lazy collection cache: one ChromaDB collection per projectId
    collections = new Map();
    constructor(client, embedder) {
        this.client = client;
...
// Helpers
// ---------------------------------------------------------------------------
function chunkToMetadata(chunk) {
    const { id, content, embedding, ...rest } = chunk;
    void id;
    void content;
    void embedding; // stored in dedicated Chroma fields
    return rest;
}
function rowsToChunks(ids, documents, metadatas) {
    const chunks = [];
    for (let i = 0; i < ids.length; i++) {
        ...
        if (id === null || content === null || meta === null)
            continue;
        chunks.push({
            id,
            content,
...
//# sourceMappingURL=semantic-memory-store.js.map