...
export function createCritiqueApp(options = {}) {
    const app = new Hono();
        ...
        app.use('/v1/*', async (c, next) => {
            ...
            if (entry && entry.resetAt > now) {
                if (entry.count >= limit) {
                    return c.json({ error: { message: 'Rate limit exceeded', type: 'rate_limit' } }, 429);
                }
                entry.count++;
            }
            else {
    ...
    app.post('/v1/review', async (c) => {
        ...
        if (!options.pipeline) {
            return c.json({ error: { message: 'No critique pipeline configured', type: 'config_error' } }, 503);
        }
        ...
        return c.json({
            ...
            findings: result.results.flatMap(r => r.findings.map(f => ({
