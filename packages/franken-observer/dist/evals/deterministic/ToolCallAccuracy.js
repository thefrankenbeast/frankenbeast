 ...
 */
export class ToolCallAccuracyEval {
    ...
    run(input) {
        ...
        if (missingParams.length > 0)
            issues.push(`missing required params: ${missingParams.join(', ')}`);
        if (ghostParams.length > 0)
            issues.push(`ghost params detected: ${ghostParams.join(', ')}`);
        if (issues.length > 0) {
            ...
            const reasonParts = [];
            if (missingParams.length > 0)
                reasonParts.push(`Missing required param(s): ${missingParams.join(', ')}.`);
            if (ghostParams.length > 0)
                reasonParts.push(`Ghost param(s) detected: ${ghostParams.join(', ')}.`);
            return {
                evalName: this.name,
                status: 'fail',
                score,
...
//# sourceMappingURL=ToolCallAccuracy.js.map