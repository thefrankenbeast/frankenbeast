 ...
 * ```
 */
export function generateGrafanaDashboard(options = {}) {
    ...
    const panels = [
        ...
        timeseries(4, 'Token Burn Rate', [{ expr: 'rate(franken_observer_tokens_total[5m])', legendFormat: '{{model}} {{type}}' }], { h: 8, w: 16, x: 0, y: 4 }, dsUid),
...
function target(expr, legendFormat, refId, dsUid) {
    return { datasource: ds(dsUid), expr, legendFormat, refId };
}
function stat(id, title, expr, gridPos, dsUid, unit) {
    return {
        id,
        type: 'stat',
        title,
...
function timeseries(id, title, targets, gridPos, dsUid) {
    return {
        id,
        type: 'timeseries',
        title,
        datasource: ds(dsUid),
        targets: targets.map((t, i) => target(t.expr, t.legendFormat, String.fromCharCode(65 + i), dsUid)),
        gridPos,
        fieldConfig: {
...
function piechart(id, title, expr, legendFormat, gridPos, dsUid) {
    return {
        id,
        type: 'piechart',
        title,
...
function bargauge(id, title, expr, legendFormat, gridPos, dsUid) {
    return {
        id,
        type: 'bargauge',
        title,
...
//# sourceMappingURL=GrafanaDashboard.js.map